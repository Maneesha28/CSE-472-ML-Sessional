...
def generate_random_symmetric_invertible_matrix(n):
    A = np.random.randint(-10, 10,  size=(n, n))
    A = A + A.T

    #making it diagonally dominant to ensure invertibility
    for i in range(n):
...
def eigen_decomposition_reconstruction(eigenvalues, eigenvectors):
    A_reconstructed = np.dot(np.dot(eigenvectors, np.diag(eigenvalues)), eigenvectors.T)
...
A = generate_random_symmetric_invertible_matrix(n)
